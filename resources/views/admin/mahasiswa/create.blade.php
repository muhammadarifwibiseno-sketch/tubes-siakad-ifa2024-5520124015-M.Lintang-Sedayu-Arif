...
<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Mahasiswa</h2></x-slot>
    <div class="py-12"><div class="max-w-2xl mx-auto sm:px-6 lg:px-8"><div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <form action="{{ route('admin.mahasiswa.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="npm" value="{{ old('npm') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" required maxlength="10">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" value="{{ old('nama') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" required maxlength="50">
            ...
            <p class="text-sm text-gray-500 mb-4">Isi bagian ini jika Anda ingin mengatur email dan kata sandi kustom untuk akun mahasiswa ini. Biarkan kosong untuk menggunakan nilai bawaan (Email: [NPM]@siakad.com, Password: password).</p>
            
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Email Akun</label>
                <input type="email" name="email" value="{{ old('email') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" placeholder="Biarkan kosong untuk email bawaan">
                @error('email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700">Password</label>
                <input type="password" name="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" placeholder="Biarkan kosong untuk password bawaan (password)">
